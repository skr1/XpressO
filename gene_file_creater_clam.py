...
def main(args):
    # Load the DataFrame
    ald_gene = pd.read_csv(args.csv_file, delimiter='\t')
    ...
    for file_path in all_files:
        ...
        if gene_id in ald_gene['gene_name'].values:
            # Get the binarized value for this gene ID
            binarized_value = ald_gene.loc[ald_gene['gene_name'] == gene_id, 'binarized'].values[0]

...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
    ...
    with torch.inference_mode():
        if isinstance(model, (CLAM_SB, CLAM_MB)):
            ...
            # predicted_label = reverse_label_dict[Y_hat]
            # if predicted_label == 'high_expreession_genes':
            #     print(f"Predicted class is 'high_expression_genes' (1)")
            # elif predicted_label == 'low_expression_genes':
            #     print(f"Predicted class is 'low_expression_genes' (0)")
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        if not slide_name.endswith(data_args.slide_ext):
            slide_name += data_args.slide_ext.lstrip('')
